...
```
## 成员节点
* 网络节点(Network Node)是指计算机网络中的一个关键组成部分 通常是一台计算机, 服务器, 路由器, 交换机等网络设备. 每个网络节点都有一个唯一的标识符, 并且在网络拓扑结构中具有特定的位置和功能.
* 在ARKFUTURE社区中, 我们根据网络节点(Network Node)的思想引入了成员节点(Member node)的概念. 成员节点是指在分布式资源共享与互相学习及互相协作的技术社区中, 具有独立技术(技能), 并参与分享技术经验, 资源和知识交换与协作的成员; 每个成员节点既是资源的贡献者(Provider), 又是资源的消费者(Consumer), 形成去中心化的协作生态.
* 在分布式资源共享与互相学习及互相协作的技术社区中, 每个成员都可作为一个独立的节点, 通过分享自己的技术, 资源和知识, 与其他成员建立联系, 并通过互相学习和协作, 达到共赢的目的. 
* 成为成员节点, 你可能需要:
```
1 个人意愿确认(主要)
* 确保你拥有独立的技术(技能)或资源
...
* **关于公钥**: 申请担任管理人员时, 您需使用个人**用户名**及**邮箱(无要求)**生成一个**OpenPGP密钥对**, 并将生成的**公钥**导出提交至**keys.openpgp.org**, 然后向**管理运维工作组**提交您记录的**公钥ID**, 以便公示. 所生成的**OpenPGP密钥对**有效期应为 90day-360day 的短期密钥(永久时效的密钥对我们不会认可)(为了安全), 到期请及时续期. 
