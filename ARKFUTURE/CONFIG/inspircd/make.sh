...
echo "开始执行编译脚本"
apt-get update
apt-get install -y build-essential wget libssl-dev libpq-dev pkg-config libargon2-dev libmaxminddb-dev libldap2-dev
wget https://github.com/inspircd/inspircd/archive/refs/tags/v4.5.0.tar.gz
tar -xzf v4.5.0.tar.gz 
rm -rf v4.5.0.tar.gz
mv inspircd-4.5.0 inspircd
cd inspircd
./configure --enable-extras ssl_openssl,ssl_gnutls,argon2,geo_maxmind,ldap
./configure --system
sleep 3
make install -j6
sleep 3
