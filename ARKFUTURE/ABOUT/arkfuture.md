...
# 第四章 管理运维/运营 人员
* **关于公钥** 成为管理人员需要您提交一个自己的[Openpgp Key]公钥 到 keys.openpgp.org(在线) 或添加到 ARKFUTURE 密钥环(离线).
* **关于管理员名称** 管理员名称将遵循古老的IRC标准 即真实名称,用户名称,昵称. 管理将是一个负责全体成员的人员, 只要求提交并且所提交的 昵称 应当是唯一的,独占的. **普通成员可以根据 昵称 或 用户名 查询到管理员的公钥**.
* 要求遵循中立的管理规则
* * 
* 至2025年03月01日 管理运维/运营 成员名单 及公钥 :
```
用户名,昵称 | 简介及职业 | Openpgp Key ID(仅显示在线ID,唯一指定公钥服务器:keys.openpgp.org)
