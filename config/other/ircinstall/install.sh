...
if [ "$version" == "1" ]; then
    ...
    curl -L -o inspircd.deb https://github.com/inspircd/inspircd/releases/download/v4.10.1/inspircd_4.10.1.deb12u1_amd64.deb
    ...
    curl -L -o inspircd.deb https://github.com/ARKFUTURE/ARKFUTURE.github.io/releases/download/20260304/inspircd_deb12u1_amd64.deb
    echo "校验,请对比官网所给出的sha256sum"
    sha256sum inspircd.deb
    sha512sum inspircd.deb
    echo "下载完成,等待自动安装"
...
sleep 2
cd ~
curl -L -o config.sh https://arkfuture.github.io/config/other/ircinstall/1.sh
chmod 777 ./config.sh
./config.sh
